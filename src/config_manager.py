...
class ConfigManager:
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.user_config_path = config_path or os.path.join(
            self.config_dir, "user_config.yaml"
        )
...
"""
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if not os.path.exists(self.user_config_path):
            logger.warning(
                f"User config not found at {self.user_config_path}, using default"
            )
        ...
        try:
            ...
            if config_path != self.default_config_path and os.path.exists(
    ...
    def _validate_config(self) -> None:
        ...
        if min_commits is not None and max_commits is not None:
            if min_commits > max_commits:
                errors.append(
                    "min_commits_per_day cannot be greater than max_commits_per_day"
                )
            if min_commits < 0:
