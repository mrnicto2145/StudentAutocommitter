        ...
        try:
            ...
            if GITPYTHON_AVAILABLE:
                ...
                if not self._is_git_repo_command_line():
                    ...
                    logger.info(
                        f"Initialized new Git repository (command line) at: {self.repo_path}"
                    )
        ...
        try:
            ...
            if not os.path.exists(readme_path):
                with open(readme_path, "w", encoding="utf-8") as f:
                    ...
                    f.write(
                        "This repository is managed by Student Autocommiter tool.\n"
                    )
            ...
            if GITPYTHON_AVAILABLE and self.repo:
                ...
                subprocess.run(
                    ["git", "add", "README.md"], cwd=self.repo_path, check=True
                )
                subprocess.run(
    ...
    def _cleanup_failed_commit(self, changed_files: List[str]) -> None:
        ...
        try:
            if GITPYTHON_AVAILABLE and self.repo:
                ...
                subprocess.run(
                    ["git", "clean", "-fd"], cwd=self.repo_path, capture_output=True
                )
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            if GITPYTHON_AVAILABLE and self.repo:
                ...
                subprocess.run(
                    ["git", "gc", "--auto"], cwd=self.repo_path, capture_output=True
                )
            ...
            if GITPYTHON_AVAILABLE and self.repo:
                ...
                if stats["total_commits"] > 0:
                    ...
                    stats["first_commit_date"] = (
                        first_commit.committed_datetime.isoformat()
                    )
                    stats["last_commit_date"] = (
                        last_commit.committed_datetime.isoformat()
                    )
