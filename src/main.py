...
def start_command(config_path, background=False, test_mode=False):
    ...
    try:
        ...
        if not repo_path or repo_path == "./workspace":
            ...
            print(
                "  Run: python main.py config set repository.local_path /path/to/your/repo"
            )
            return False
        ...
        if status["last_commit"]:
            print(
                f"✓ Last commit: {status['last_commit']['hash']} - {status['last_commit']['message'][:50]}..."
            )
...
def install_service_command(config_path, method="auto"):
    ...
    try:
        ...
        if result.get("success", False):
            print(
                f"✓ Service installed successfully using {result.get('method', 'unknown')}"
            )
...
def stats_command(config_path):
    ...
    try:
        ...
        if errors:
            for error in errors:
                print(
                    f"  - {error['timestamp']}: {error['error_type']} - {error['error_message'][:50]}..."
                )
        else:
...
def status_command(config_path):
    ...
    try:
        ...
        if status["last_commit"]:
            print(
                f"Last commit: {status['last_commit']['hash']} - {status['last_commit']['message']}"
            )
...
Use responsibly and ethically!
    ...
    config_parser.add_argument(
        "action",
        choices=["show", "get", "set", "reset", "reload", "validate", "path"],
    ...
    history_parser.add_argument(
        "--limit", "-l", type=int, default=10, help="Number of commits to show"
    )

    # Команда uninstall-service
    uninstall_parser = subparsers.add_parser(
        "uninstall-service", help="Remove startup service"
    )
