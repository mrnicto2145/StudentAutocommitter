...
class ErrorHandler:
    ...
    def handle_commit_error(
        self, error: Exception, commit_data: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _add_to_pending_queue(
        self, commit_data: Dict[str, Any], error_info: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def process_pending_commits(self, retry_callback) -> Dict[str, Any]:
        ...
        """
        stats = {"processed": 0, "succeeded": 0, "failed": 0, "removed": 0}
                ...
                if datetime.now() >= next_retry:
                    ...
                    try:
                        ...
                        if result.get("success", False):
                            ...
                            stats["removed"] += 1
                            logger.info(
                                f"Retry succeeded for commit from {item['added_at']}"
                            )
                        else:
                            ...
                            if item["retry_count"] < item["max_retries"]:
                                ...
                                stats["failed"] += 1
                                logger.error(
                                    f"Max retries reached, removing from queue"
                                )
    ...
    def _update_stats(
        self, successful: bool = False, failed: bool = False, count: int = 1
    ):
        ...
        try:
            if os.path.exists(self.stats_file):
                ...
                if stats.get("total_commits", 0) > 0:
                    success_rate = (
                        stats.get("successful_commits", 0) / stats["total_commits"]
                    ) * 100
